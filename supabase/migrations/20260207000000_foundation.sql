...
-- Viewing other league members' roles is done via SECURITY DEFINER RPCs
-- to avoid self-referential subquery recursion in RLS policies.
create policy "Users can view own roles"
  on user_roles for select
  to authenticated
  using (user_id = auth.uid());

-- NOTE: Split into INSERT/UPDATE/DELETE (not FOR ALL) to avoid infinite
-- recursion with the SELECT policy's self-referential subquery.

create policy "Commissioners can insert user roles"
...
create policy "Commissioners can update user roles"
...
create policy "Commissioners can delete user roles"
