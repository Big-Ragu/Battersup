...
import { GripVertical, Save, UserPlus, X } from 'lucide-react';
    ...
    >
      ...
      <div
        className={`flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full text-xs font-bold ${
          isStarter ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-600'
        }`}
      >
        {index + 1}
      </div>
      ...
      {isStarter ? (
        ...
        >
          ...
          {Object.entries(FIELD_POSITIONS).map(([num, label]) => {
            const n = Number(num);
            const isUsed =
              usedPositions.includes(n) && slot.fielding_position !== n;
            return (
...
export function LineupEditor({
  ...
  const hasDefault =
    currentLineup.length === 0 && (defaultLineup ?? []).length > 0;

  const initialLineup =
    currentLineup.length > 0
      ? currentLineup.sort((a, b) => a.batting_order - b.batting_order)
      : hasDefault
        ? defaultLineup!.sort((a, b) => a.batting_order - b.batting_order)
        : makeEmptyLineup(Math.max(9, roster.length));

  const [lineup, setLineup] = useState<LineupSlot[]>(initialLineup);
  const [showDefaultBanner, setShowDefaultBanner] = useState(hasDefault);
  ...
  async function handleSave() {
    ...
    const { error: rpcError } = await supabase.rpc('set_game_lineup', {
      p_game_id: gameId,
      p_team_id: teamId,
      p_lineup: lineupData,
    });
  ...
  return (
    <div className="mx-auto max-w-lg">
      {showDefaultBanner && (
        <div className="mb-3 rounded-md border border-blue-200 bg-blue-50 px-4 py-2 text-sm text-blue-800">
          Pre-populated from standard lineup. Save to confirm for this
          game.
          <button
      ...
      {success && (
        <p className="mt-3 text-sm text-green-600">Lineup saved!</p>
      )}
    </div>
  );
}
